...
Area2i ProgressBar::preferredSize(NVGcontext *) const {
    return Area2i(70, 12);
}
...
void ProgressBar::draw(NVGcontext* ctx) {
    ...
    NVGpaint paint = nvgBoxGradient(
        ...
        mSize.w()-2, mSize.h(), 3, 4, Color(0, 32), Color(0, 92));
    nvgBeginPath(ctx);
    nvgRoundedRect(ctx, mPos.x(), mPos.y(), mSize.w(), mSize.h(), 3);
    nvgFillPaint(ctx, paint);
    nvgFill(ctx);

    float value = std::min(std::max(0.0f, mValue), 1.0f);
    int barPos = (int) std::round((mSize.w() - 2) * value);

    paint = nvgBoxGradient(
        ctx, mPos.x(), mPos.y(),
        barPos+1.5f, mSize.h()-1, 3, 4,
